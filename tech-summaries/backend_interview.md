...
Влияние GIL
На потоки (threading)
GIL действует внутри одного процесса. Все потоки этого процесса конкурируют за один GIL.

I/O-операции — поток отпускает GIL пока ждёт → другие потоки работают → параллелизм есть
CPU-операции — поток держит GIL пока считает → другие потоки заблокированы → параллелизма нет

На процессы (multiprocessing)
Каждый процесс — это отдельный интерпретатор Python со своим GIL.
Процессы друг другу не мешают. Если у тебя 4 процесса на 4-ядерном CPU — все четыре могут выполнять Python-код одновременно на разных ядрах.

threading  → один GIL на всех → параллелизм только при I/O
multiprocessing → у каждого свой GIL → настоящий параллелизм
